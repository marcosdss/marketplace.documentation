...
Se tiver qualquer dúvida quanto a suas seções, consulte a **documentação do OrderForm**.
...
Veja a **documentação do OrderForm** para descobrir quais são as seções.
...
Consulte a **documentação do OrderForm** para mais detalhes sobre ele.
...
Um item a ser adicionado é obrigatoriamente composto por: `id`, `quantity` e `seller`. A propriedade `id` pode ser obtida pelo **Catalog**, observando o itemId do item no Array de items do produto.
...
| Nome                    | Tipo                          |
| -----------------------:| :-----------------------------|
| **items** | **Array** o conjunto de items que vão ser adicionados. Mesmo que só haja um item, deve ser envolto num Array.|
| **salesChannel** | **Number ou String** (Parâmetro opcional, default = `1`) |
...
Veja a **documentação do OrderForm** para conhecer mais sobre o que compõe o objeto de item.
...
| **splitItem** | **Boolean** Default: true Informa se um item separado deve ser criado caso os items a serem atualizados tenham anexos/serviços incluídos.|
...
| Nome                    | Tipo                          |
| -----------------------:| :-----------------------------|
| **itemIndex** | **Number** o índice do item ao qual a oferta se aplica |
| **newItemsOptions** | **Array** (Opcional) Propriedades que devem ser atribuídas aos novos items|
...
| Nome                    | Tipo                          |
| -----------------------:| :-----------------------------|
| **shippingData** | **Object** que contenha a informação de envio com `logisticsInfo` e `selectedAddresses`. |
| **orderFormId** | **String** contendo o Id do `orderForm` |
| **country** | **String** a sigla de 3 letras do país, por exemplo, "BRA" |
| **salesChannel** | **Number ou String** (Parâmetro opcional, default = `1`) |
...
Certifique-se disso com a propriedade canEditData do orderForm. Veja a **documentação do OrderForm**.
...
| -----------------------:| :-----------------------------|
| **email** | **String** |
| **salesChannel** | **Number ou String** (default = `1`) |
...
| Nome                    | Tipo                          |
| -----------------------:| :-----------------------------|
| **offeringId** | **String ou Number** pode ser encontrado na propriedade `id` da offering |
| **itemIndex** | **Number** o índice do item ao qual a oferta se aplica |
...
| Nome                    | Tipo                          |
| -----------------------:| :-----------------------------|
| **itemIndex** | **Number** o índice do item a ser incluído o anexo |
| **attachmentName**  | **String**  pode ser encontrado na propriedade `name` em attachmentOfferings dentro do objeto do item |
| **content** | **Object** um objeto que respeite o schema descrito na propriedade `schema` em attachmentOfferings |
| **splitItem** | **Boolean** Default: true Informa se um item separado deve ser criado caso os items a serem atualizados tenham anexos incluídos.|
...
| Nome |   Tipo   |                          Descrição                          |
|:---------:|:--------:|:-------------------------------------------------------------:|
|  **itemIndex**  | **Number** | o índice do item que o serviço se aplica |
| **bundleId**  | **String ou Number** | pode ser encontrado na propriedade `id` do bundleItem |
| **attachmentName**  | **String** | pode ser encontrado na propriedade `name` em attachmentOfferings dentro do objeto do serviço |
| **content** | **Object** | um objeto que respeite o schema descrito na propriedade `schema` em attachmentOfferings |
...
| Nome |   Tipo   |                          Descrição                          |
|:---------:|:--------:|:-------------------------------------------------------------:|
| **criteria** | **String** | `name` ou `add_time` |
| **ascending**  | **Boolean**  | `true` para crescente e `false` para decrescente |
...
| Nome |   Tipo   |                          Descrição                          |
|:---------:|:--------:|:-------------------------------------------------------------:|
| **items** | **Array** | objeto com o sku a ser removido em quantidade 0, e o novo sku que será adicionado. Deve estar em volto em um array|
| **splitItem** | **Boolean** | Default: true Informa se um item separado deve ser criado caso os items a serem atualizados tenham anexos/serviços incluídos.|
...
| Nome |   Tipo   |                          Descrição                          |
|:---------:|:--------:|:-------------------------------------------------------------:|
| **orderGroupId** | **number** | id do pedido a ser gerado no momento da finalização da compra |
...
[Vtex - Checkout](https://github.com/vtex/vtex.js/blob/master/docs/checkout/README.md)
